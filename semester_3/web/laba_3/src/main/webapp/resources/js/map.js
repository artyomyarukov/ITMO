...
function syncRValue() {
    ...
    const rOutput = document.getElementById('mainForm:rOutput');

    if (rOutput && rOutput.innerHTML) {
        currentR = parseFloat(rOutput.innerHTML);
    } else {

        const rInput = document.getElementById('mainForm:rSliderInput');
        if (rInput && rInput.value) {
            currentR = parseFloat(rInput.value);
        }
    }
    console.log("Текущее R синхронизировано: ", currentR);
}
...
function updateR() {
    ...
    if (rInput) {
        ...
        console.log("R успешно обновлен через AJAX: ", currentR);


        redrawGraph();
    } else {
        console.error("Критическая ошибка: Скрытое поле R не найдено!");
    }
}
...
// Сектор (Четверть круга)
    ...
    ctx.arc(centerX, centerY, (currentR) * scale, Math.PI, -Math.PI/2, false);
    ctx.fill();
...
// Прямоугольник
    ctx.beginPath();
    ctx.rect(centerX, centerY - currentR * scale, (currentR / 2) * scale, currentR * scale);
    ctx.fill();
    ctx.closePath();
    ...
    // 3. Деления осей (Простые метки, как были)
    ctx.font = '10px Arial';
    ctx.fillStyle = 'black';
...
function handleCanvasClick(event, canvas) {
    ...
    console.log(`Click: ${x.toFixed(7)}, ${y.toFixed(7)}`);
    ...
    if (xInput && yInput && rInput) {
        xInput.value = x.toFixed(7);
        yInput.value = y.toFixed(7);
